...
switch AssemblyChoice
    ...
    case 2
        pat2 = [pat 'KDE_binsTaskonly' filesep 'MEDIUMTaskonly' filesep];
    case 3
        pat2 = 'C:\Analysis\AssemblyAnalysis\Sleep\Task\';
end

mkdir([pat 'MixedSelectivity_MEDIUM'])
...
clear p_ q_ q_names q_names FR SC LR CE p_thresh
%% Save results
        fnOut = sprintf('%sMixedSelectivity_MEDIUM%s%s_%s_MixedSelectivity_Units.mat',pat,filesep,fname,Areas{iArea});
        save(fnOut,'D','tbDelay_2','tbDelay_4','tbDelay_6','tbDelay_8','tbAll','-v7.3');
...
%         
    ...
    %% Save
    fnOut = sprintf('%sMixedSelectivity_MEDIUM%s%s_MixedSelectivity_Ass.mat',pat,filesep,fname);
    save(fnOut,'D_Ass','tbDelay_2','tbDelay_4','tbDelay_6','tbDelay_8','tbAll','-v7.3');
    fprintf('Done.\n')
    clear D_Ass
end
clearvars -except Nbs AssemblyChoice color_ Delays_ bw tbShort tbAll tbMedium tbLong tbANOVA  pat fileList fileListAss Areas tlimsANOVA tlimsAll tbShort tbMedium tbLong  shift normaliseFscores normWin 
