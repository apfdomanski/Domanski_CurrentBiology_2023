...
%                             nu = [length(PFCcells),length(HPcells)];
                            ...
                            if ResampleTS
                                for nrep = 1:Nbs_TScore
                                    fprintf('Decoding file %d/%d (%s) %s units, %s %s trials, F-decoder... bs %d/%d \n',iFile,length(fileList),fname,Areas{iArea},Outcome{iOutcome}, Delays_{iDelay},nrep,Nbs_TScore)
