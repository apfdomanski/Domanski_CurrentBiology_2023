...
for iFile =1:length(fileList)
    ...
    for iArea = 1:2%length(Areas)
        ...
        %% sanity check: Decode Long L/R from original timestamps
        
        % %%%%%%%%%%%%% Sample
        % Ltrials = [];nL = 0;tlimsLong_=[];
        % Lefttrials = trangeleft_sample(:,1)+5000000;
        % for iTrial = 1:length(Lefttrials)
        %     try
        %         tlimsLong_ = Lefttrials(iTrial)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        %         Ltrials = [Ltrials;iFR_(tlimsLong_(1):tlimsLong_(1)+length(tbLong)-1,:)];
        %         nL=nL+1;
        %     end
        % end
        % Rtrials = []; nR=0; tlimsLong_=[];
        % Righttrials = trangeright_sample(:,1)+5000000;
        % for iTrial =1:length(Righttrials)
        %     try
        %         tlimsLong_ = Righttrials(iTrial,1)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        %         Rtrials = [Rtrials;iFR_(tlimsLong_(1):tlimsLong_(1)+length(tbLong)-1,:)];
        %         nR=nR+1;
        %     end
        % end
        % [D.Long.LR.avgFR,D.Long.LR.seFR,D.Long.LR.Ft2,D.Long.LR.Rt2,D.Long.LR.Ft2ciL,D.Long.LR.Ft2ciH,D.Long.LR.TS,D.Long.LR.dfnum,D.Long.LR.dfden] = DecodeStats([Ltrials;Rtrials],[ones(nL,1);2*ones(nR,1)],0.05);
        %
        % figure; hold on
        % plot(tbLong,D.Long.LR.Ft2,'b')
        %
        % %Decode Long L/R as before
        % Ltrials = [];nL = 0;
        % Lefttrials = [t.Long.SamplePress_LeftCorrect;t.Medium.SamplePress_LeftCorrect];
        %
        % for iTrial =1:length(Lefttrials)
        %     try
        %         tlimsLong_ = Lefttrials(iTrial)/1e6+tlimsLong;
        ...
        % end
        % Rtrials = []; nR=0;
        % Righttrials = [t.Long.SamplePress_RightCorrect;t.Medium.SamplePress_RightCorrect];
        % for iTrial =1:length(Righttrials)
        %     try
        %         tlimsLong_ = Righttrials(iTrial)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        %         Rtrials = [Rtrials;iFR_(tlimsLong_(1):tlimsLong_(1)+length(tbLong)-1,:)];
        %         nR=nR+1;
        %     end
        ...
        % [D.Long.LR.avgFR,D.Long.LR.seFR,D.Long.LR.Ft2,D.Long.LR.Rt2,D.Long.LR.Ft2ciL,D.Long.LR.Ft2ciH,D.Long.LR.TS,D.Long.LR.dfnum,D.Long.LR.dfden] = DecodeStats([Ltrials;Rtrials],[ones(nL,1);2*ones(nR,1)],0.05);
        %
        % plot(tbLong,D.Long.LR.Ft2,'r')
        %
        %  %%%%%%%%% Choice
        % Ltrials = [];nL = 0;tlimsLong_=[];
        % Lefttrials = trangeleft_choice(:,1)+5000000;
        % for iTrial = 1:length(Lefttrials)
        %     try
        %         tlimsLong_ = Lefttrials(iTrial)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        ...
        % end
        % Rtrials = []; nR=0; tlimsLong_=[];
        % Righttrials = trangeright_choice(:,1)+5000000;
        % for iTrial =1:length(Righttrials)
        %     try
        %         tlimsLong_ = Righttrials(iTrial,1)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        ...
        %
        % Ltrials = [];nL = 0;
        % Lefttrials = [t.Long.ChoicePress_LeftCorrect;t.Medium.ChoicePress_LeftCorrect];
        %
        % for iTrial =1:length(Lefttrials)
        %     try
        %         tlimsLong_ = Lefttrials(iTrial)/1e6+tlimsLong;
        ...
        % end
        % Rtrials = []; nR=0;
        % Righttrials = [t.Long.ChoicePress_RightCorrect;t.Medium.ChoicePress_RightCorrect];
        % for iTrial =1:length(Righttrials)
        %     try
        %         tlimsLong_ = Righttrials(iTrial)/1e6+tlimsLong;
        %         tlimsLong_ = closest(Tmtx,tlimsLong_);
        ...
        % [D.Long.LR.avgFR,D.Long.LR.seFR,D.Long.LR.Ft2,D.Long.LR.Rt2,D.Long.LR.Ft2ciL,D.Long.LR.Ft2ciH,D.Long.LR.TS,D.Long.LR.dfnum,D.Long.LR.dfden] = DecodeStats([Ltrials;Rtrials],[ones(nL,1);2*ones(nR,1)],0.05);
        %
        % plot(tbLong,D.Long.LR.Ft2,'r')%% Plot L/R decoding
...
%                  plot(D_.CVE)
        ...
        if plotOnline
            figure; hold on
            plot([0:(length(tbAll)*2-1)]*bw,D.Short.LR.Ft2,'r')
            plot([0:(length(tbAll)*2-1)]*bw,D.Medium.LR.Ft2,'g')
            plot([0:(length(tbAll)*2-1)]*bw,D.Long.LR.Ft2,'b')
        ...
        if plotOnline
            figure; hold on
            plot([0:(length(tbAll)*2-1)]*bw,mean([D.Short.CE_L.Ft2;D.Short.CE_R.Ft2]),'r')
            plot([0:(length(tbAll)*2-1)]*bw,mean([D.Medium.CE_L.Ft2;D.Medium.CE_R.Ft2]),'g')
            plot([0:(length(tbAll)*2-1)]*bw,mean([D.Long.CE_L.Ft2;D.Long.CE_R.Ft2]),'b')
            legend({'Short delay (4s)','Medium delay (8s)','Long delay (16s)'})
            % plot(tbShort,D.ShortLR.Ft2ciH,':r')
            % plot(tbMedium,D.Medium.Ft2ciH,':g')
            % plot(tbLong,D.Long.Ft2ciH,':b')
        ...
        if plotOnline
            figure; hold on
            plot(tbShort,   mean([D.Short.SC_L.Ft2;D.Short.SC_R.Ft2]),'r')
            plot(tbMedium, mean([D.Medium.SC_L.Ft2;D.Medium.SC_R.Ft2]),'g')
            plot(tbLong,    mean([D.Long.SC_L.Ft2;D.Long.SC_R.Ft2]),'b')
            legend({'Short delay (4s)','Medium delay (8s)','Long delay (16s)'})
        end
        %% Trial Fano factor
         for iDelay =1:length(Delays_)
...
%                 sum((sum(p_(:,4:7)'<0.05)>1))];    % Mixed tuning
        ...
        %% Save results
        fnOut = sprintf('%sMixedSelectivity_LONG%s%s_%s_MixedSelectivity_Units.mat',pat,filesep,fname,Areas{iArea});
...
%                          D_.CVEindividual(iAss,:) = DecodeCVE_mex(FSC_(:,iAss),evt0,0.05);
                         D_.CVEindividual(iAss,:) = DecodeCVE(FSC_(:,iAss),evt0,0.05);
                     end
                     if runCVEdecoder
