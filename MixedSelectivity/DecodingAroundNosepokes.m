...
for iFile =1:length(fileList)
    ...
    for iArea = 1:2%length(Areas)
        ...
        iFR_ = iFR;
        % iFR_ = zscore(iFR_);
        %% NosePoke Decoding for units
        for iDelay =1:length(Delays_)
            
            eval(sprintf('LeftTrials = [t.%s.NosePoke_LeftCorrect];',Delays_{iDelay}));
            eval(sprintf('RightTrials = [t.%s.NosePoke_RightCorrect];',Delays_{iDelay}));
            eval(sprintf('LeftTrialsE = [t.%s.NosePoke_LeftError''];',Delays_{iDelay}));
            eval(sprintf('RightTrialsE = [t.%s.NosePoke_RightError''];',Delays_{iDelay}));
...
%             end
            if exist('D_')
                eval(sprintf('D.%s.NP =  D_;',Delays_{iDelay}))
            end
            clear D_
...
%              end
             if exist('D_')
                 eval(sprintf('D.%s.NP_err =  D_;',Delays_{iDelay}))
             end
             clear D_
        ...
        if exist('D')
            ...
            fnOut = sprintf('%sMixedSelectivity%s%s_%s_NosePokeDecoding_Units.mat',pat,filesep,fname,Areas{iArea});
...
%                 if nL>2 && nR>2
                    ...
                    for i=1:nrep
                        ...
                        D_.NP{iArea}.CVE(i,:) = DecodeCVE_mex(FSC_(:,rs{i}),evt0,0.05);
                    end
                    D_.NP{iArea}.CVE = nanmean(D_.NP{iArea}.CVE,1);
...
%                 if nLe>2 && nRe>2
                    ...
                    for i=1:nrep
                        rs{i} = randsample(size(Ltrials,2),min(nAss_));
                        D_.NP{iArea}.CVE_err(i,:)=DecodePredErr(FSC_(:,rs{i}),evt0,0.05);
                    end
                    D_.NP{iArea}.CVE_err = nanmean(D_.NP{iArea}.CVE_err,1);
...
%                 end
        ...
        if exist('D_')
            eval(sprintf('D_Ass.%s.NP =  D_.NP;',Delays_{iDelay}))
        end
    ...
    if exist('D_Ass')
        ...
        fnOut = sprintf('%sMixedSelectivity%s%s_NosePokeDecoding_Units.mat',pat,filesep,fname);
...
for iArea = 1:2%length(Areas)
    ...
    for iFile =1:length(fileList)
        ...
        fname=strtok(fileList(iFile).name,'_');
        fnIn = sprintf('%s\\MixedSelectivity\\%s_%s_NosePokeDecoding_Units.mat',pat,fname,Areas{iArea});
        load(fnIn ,'D');
        ...
        for iDelay = 1:length(Delays_)
            ...
            try
                eval(sprintf('D_.%s.NP.CVE{iArea}(:,iFile)      = D.%s.NP.CVE;',Delays_{iDelay},Delays_{iDelay}))
            catch
                 eval(sprintf('D_.%s.NP.CVE{iArea}(:,iFile)     = nan(length(tbAll),1);',Delays_{iDelay}))
            end
            try
                eval(sprintf('D_.%s.NP_err.CVE{iArea}(:,iFile)  = D.%s.NP_err.CVE;',Delays_{iDelay},Delays_{iDelay}))
            catch
                eval(sprintf('D_.%s.NP_err.CVE{iArea}(:,iFile)  = nan(length(tbAll),1);',Delays_{iDelay}))
            end
...
for iFile = 1:length(fileListAss)
        try
        fname=strtok(fileListAss(iFile).name,'_');
        fnIn = sprintf('%s\\MixedSelectivity\\%s_NosePokeDecoding_Units.mat',pat,fname);
        load(fnIn ,'D_Ass');
        ...
        for iArea = 1:3
            for iDelay = 1:length(Delays_)
                ...
                % L/R: Continuous decoding of Positional information - correct trials
                flag = 0;
                if eval(sprintf('isfield(D_Ass.%s,''NP'')',Delays_{iDelay})) 
                    if eval(sprintf('~isempty(D_Ass.%s.NP{iArea})',Delays_{iDelay}))
                        flag = 1;
                        eval(sprintf('D_temp = D_Ass.%s.NP{iArea};',Delays_{iDelay}))
                     
                        
                         try
                            eval(sprintf('D_Ass_.%s.NP.CVE{iArea}(:,iFile)      = D_temp.CVE;',Delays_{iDelay}))
                         catch                         
                             eval(sprintf('D_Ass_.%s.NP.CVE{iArea}(:,iFile)     = nan(length(tbAll),1);',Delays_{iDelay}))
                         end
                         try
                            eval(sprintf('D_Ass_.%s.NP.CVE_err{iArea}(:,iFile)      = D_temp.CVE_err;',Delays_{iDelay}))
                         catch                         
                             eval(sprintf('D_Ass_.%s.NP.CVE_err{iArea}(:,iFile)     = nan(length(tbAll),1);',Delays_{iDelay}))
                         end
...
for iDelay = 1:length(Delays_)
    for iArea = 1:2
        eval(sprintf('D_temp = D_.%s.NP.CVE{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_.%s.NP.CVE{iArea} = D_temp;',Delays_{iDelay}))
    end
    for iArea = 1:2
        eval(sprintf('D_temp = D_.%s.NP_err.CVE{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_.%s.NP_err.CVE{iArea} = D_temp;',Delays_{iDelay}))
     end
    
    for iArea = 1:3
        eval(sprintf('D_temp = D_Ass_.%s.NP.CVE{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_Ass_.%s.NP.CVE{iArea} = D_temp;',Delays_{iDelay}))
        
        eval(sprintf('D_temp = D_Ass_.%s.NP.CVE_err{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_Ass_.%s.NP.CVE_err{iArea} = D_temp;',Delays_{iDelay}))
    end
end
...
for iArea =1:2%length(Areas)
    ...
    for iDelay = 1:length(Delays_)
        ...
        eval(sprintf('corr = (1-D_.%s.NP.CVE{iArea})*100;',Delays_{iDelay}));
        eval(sprintf('err = (1-D_.%s.NP_err.CVE{iArea})*100;',Delays_{iDelay}));
...
for iArea =1:length(Areas)
    ...
    for iDelay = 1:length(Delays_)
        subplot(1,length(Delays_),iDelay);hold on
        eval(sprintf('corr = (1-D_Ass_.%s.NP.CVE{iArea})*100;',Delays_{iDelay}));
        eval(sprintf('err = (1-D_Ass_.%s.NP.CVE_err{iArea})*100;',Delays_{iDelay}));
%         corr = smooth2a(corr,10,0);
%         err = smooth2a(err,10,0);
