...
for iFile =1:length(fileList)
    ...
    for iArea = 1:2%length(Areas)
        ...
        for iDelay =1:length(Delays_)
            ...
            eval(sprintf('LeftTrialsE = [t.%s.SamplePress_LeftError'',t.%s.ChoicePress_LeftError''];',Delays_{iDelay},Delays_{iDelay}));
            eval(sprintf('RightTrialsE = [t.%s.SamplePress_RightError'',t.%s.ChoicePress_RightError''];',Delays_{iDelay},Delays_{iDelay}));
...
%             end
        if exist('D_')
            eval(sprintf('D.%s.LR_err =  D_.LR_err;',Delays_{iDelay}))
        end
        ...
        if exist('D')
            ...
            fnOut = sprintf('%sMixedSelectivity%s%s_%s_PredictionError_Units.mat',pat,filesep,fname,Areas{iArea});
...
%                 if nL>2 && nR>2
                    ...
                    for i=1:nrep
                        ...
                        D_.LR{iArea}.pe0(i,:)=DecodePredErr(FSC_(:,rs{i}),evt0,0.05);
                      
                    end
                    D_.LR{iArea}.pe0 = nanmean(D_.LR{iArea}.pe0,1);
...
%                 end
    ...
    if exist('D_Ass')
        ...
        fnOut = sprintf('%sMixedSelectivity%s%s_PredictionError_Ass.mat',pat,filesep,fname);
...
%% batch import unit data for meta-analysis and plotting - correct trials
% Import
clear D_
for iArea = 1:2%length(Areas)
...
%         fnIn =fullfile(pat,'MixedSelectivity','TrialNosMatched',sprintf('%s_%s_MixedSelectivity_Units.mat',fname,Areas{iArea}))
%         fnIn =fullfile(pat,'MixedSelectivity','AllAvailableTrials',sprintf('%s_%s_MixedSelectivity_Units.mat',fname,Areas{iArea}))
        fnIn =fullfile(pat,'MixedSelectivity',sprintf('%s_%s_MixedSelectivity_Units.mat',fname,Areas{iArea}))
        load(fnIn ,'D');
...
%% batch import unit data for meta-analysis and plotting - error trials
for iFile = 1:length(fileListAss)
        try
        for iArea = 1:2
            fname=strtok(fileListAss(iFile).name,'_');
            fnIn = sprintf('%sMixedSelectivity%s%s_%s_PredictionError_Units.mat',pat,filesep,fname,Areas{iArea});
            load(fnIn ,'D');
        for iDelay = 1:length(Delays_)
                
                % L/R: Continuous decoding of Positional information - correct trials
                flag = 0;
                if eval(sprintf('isfield(D.%s,''LR_err'')',Delays_{iDelay})) 
                    if eval(sprintf('~isempty(D.%s.LR_err{iArea})',Delays_{iDelay}))
                        flag = 1;
                        eval(sprintf('D_temp = D.%s.LR_err{iArea};',Delays_{iDelay}))
                      
                        eval(sprintf(   'D_.%s.LR_err.pe{iArea}(:,iFile)       = D_temp.pe0;',Delays_{iDelay}));
                        clear D_temp
                    end
                end
                if ~flag
                	eval(sprintf(   'D_Ass_.%s.LR_err.pe{iArea}(:,iFile)       = nan(2*length(tbAll),1);',Delays_{iDelay}));
                end
        ...
        eval(sprintf(   'D_.%s.LR_err.pe{iArea}(:,iFile)       = nan(2*length(tbAll),1);',Delays_{iDelay}));

        end
end
%% batch import Assem data for meta-analysis and plotting - correct trials
for iFile = 1:length(fileListAss)
        try
        fname=strtok(fileListAss(iFile).name,'_');
...
%% batch import Assem data for meta-analysis and plotting - error trials
for iFile = 1:length(fileListAss)
        try
        fname=strtok(fileListAss(iFile).name,'_');
        fnIn = sprintf('%s\\MixedSelectivity\\%s_PredictionError_Ass.mat',pat,fname);
        load(fnIn ,'D_Ass');
            ...
            for iDelay = 1:length(Delays_)
                ...
                if eval(sprintf('isfield(D_Ass.%s,''LR_err'')',Delays_{iDelay})) 
                    if eval(sprintf('~isempty(D_Ass.%s.LR_err{iArea})',Delays_{iDelay}))
                        ...
                        eval(sprintf(   'D_Ass_.%s.LR_err.pe{iArea}(:,iFile)       = D_temp.pe0;',Delays_{iDelay}));
                        clear D_temp
                    end
                end
                if ~flag
...
for iDelay = 1:length(Delays_)
    for iArea = 1:2
        eval(sprintf('D_temp = D_.%s.LR_err.pe{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_.%s.LR_err.pe{iArea} = D_temp;',Delays_{iDelay}))
    end
    for iArea = 1:3
        eval(sprintf('D_temp = D_Ass_.%s.LR_err.pe{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_Ass_.%s.LR_err.pe{iArea} = D_temp;',Delays_{iDelay}))
         eval(sprintf('D_temp = D_Ass_.%s.LR.CVE{iArea};',Delays_{iDelay}))
        D_temp(:,sum(D_temp)==0)=NaN;
        D_temp(:,isnan(D_temp(1,:)))=[];
        eval(sprintf('D_Ass_.%s.LR.CVE{iArea} = D_temp;',Delays_{iDelay}))
    end
end
...
for iArea =1:2%length(Areas)
    ...
    for iDelay = 1:length(Delays_)
        ...
        eval(sprintf('corr = (1-D_.%s.LR.CVE{iArea})*100;',Delays_{iDelay}));
        eval(sprintf('err = (1-D_.%s.LR_err.pe{iArea})*100;',Delays_{iDelay}));
...
for iArea =1:length(Areas)
    ...
    for iDelay = 1:length(Delays_)
        subplot(1,length(Delays_),iDelay);hold on
        eval(sprintf('corr = (1-D_Ass_.%s.LR.CVE{iArea})*100;',Delays_{iDelay}));
        eval(sprintf('err = (1-D_Ass_.%s.LR_err.pe{iArea})*100;',Delays_{iDelay}));
        corr = smooth2a(corr,10,0);
        err = smooth2a(err,10,0);
    ...
    legend({'Correct','Errors'},'Location','southwest')
