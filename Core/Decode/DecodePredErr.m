function PE=DecodePredErr(FR,evt0,reg)
...
for t=1:Ltr % Loop across each time bin
    ...
    for s=1:2 % Loop across each category for errors
        Xtest=X(evt0==10*s,:); k= ~isnan(Xtest(:,1)); Xtest=Xtest(k,:);
        n=length(k);
        ...
        PE(t)=PE(t)+sum(cpred~=s);                                         % Classifcation error at this time point
        N(t)=N(t)+n;
    end;
