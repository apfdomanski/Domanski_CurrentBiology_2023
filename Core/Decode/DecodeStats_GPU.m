function [avgFR,seFR,Ft2,Rt2,Ft2ciL,Ft2ciH,TS,dfnum,dfden]=DecodeStats_GPU(FR,evt0,reg)
...
if nargout>2
    ...
    for i=1:Ltr
        ...
        Cpool=((n1-1)*cov(gpuArray(X1))+(n2-1)*cov(gpuArray(X2)))./(n1+n2-2);
        Creg=Cpool+reg*eye(size(Cpool));
        U_=Cpool+1e-8*eye(size(Cpool));
        U=sqrtm(gather(U_));
