...
%     end
    ...
    %% 3/ Make synthetic assemblies by aggregating best Single Units and decode
...
%     D_.AssBestSUs.Members = MakebestSUAssems(D_.Units.Members.Rank,D_.Units.Members.SortedScore, Ltrials_,Rtrials_,20);
%     % Cell assembly nonmember neurons
%     fprintf('Calculating best single units: Assembly non-member neurons\n')
%     D_.AssBestSUs.Nonmembers = MakebestSUAssems(D_.Units.Nonmembers.Rank,D_.Units.Nonmembers.SortedScore, Ltrials_,Rtrials_,20);
%     % Members of joint-area assemblies
%     fprintf('Calculating best single units: Joint area assembly member neurons\n')
%     D_.AssBestSUs.JointMembers = MakebestSUAssems(D_.Units.JointMembers.Rank,D_.Units.JointMembers.SortedScore, Ltrials_,Rtrials_,20);
...
%             surf(smooth2a(D_.AssBestSUs.All.ScoreRanks{s},2,2),'EdgeColor','k','FaceColor','k','FaceAlpha',0.3)
    ...
    %% 4/ Decoding from BEST SYNTHETIC ASSEMBLIES
    
    % 1) with noise correlations intact
    
%     % All neurons
%     fprintf('Calculating best assemblies: All units\n')
%     D_.AssBestAssem.All = MakebestEnsembleAssems(B.usel_out,Ltrials_,Rtrials_,20);
%     % Cell assembly member neurons
%     fprintf('Calculating best assemblies: Assembly member neurons\n')
%     D_.AssBestAssem.Members = MakebestEnsembleAssems(globalmemberUnitsReal,Ltrials_,Rtrials_,20);
%     % Cell assembly nonmember neurons
%     fprintf('Calculating best assemblies: Assembly non-member neurons\n')
%     D_.AssBestAssem.Nonmembers = MakebestEnsembleAssems(globalnonmemberUnitsReal,Ltrials_,Rtrials_,20);
%     % Members of joint-area assemblies
%     fprintf('Calculating best assemblies: Joint area assembly member neurons\n')
%     JointOnly = false;
%     D_.AssBestAssem.JointMembers = MakebestEnsembleAssems(JointMemberUnitsReal,Ltrials_,Rtrials_,20,JointOnly); 
   
    % ... NB Last arguement skips the check for units in PFC and HP
    % individually as these are ignored for this
    
    % 2) with noise correlations smashed
    
    % All neurons
    fprintf('Calculating best assemblies: All units\n')
    D_.AssBestAssem.All_noNoiseCorrs = MakebestEnsembleAssems(B.usel_out,Ltrials_,Rtrials_,20,true);
    % Cell assembly member neurons
    fprintf('Calculating best assemblies: Assembly member neurons\n')
    D_.AssBestAssem.Members_noNoiseCorrs = MakebestEnsembleAssems(globalmemberUnitsReal,Ltrials_,Rtrials_,20,true);
    % Cell assembly nonmember neurons
    fprintf('Calculating best assemblies: Assembly non-member neurons\n')
    D_.AssBestAssem.Nonmembers_noNoiseCorrs = MakebestEnsembleAssems(globalnonmemberUnitsReal,Ltrials_,Rtrials_,20,true);
    % Members of joint-area assemblies
    fprintf('Calculating best assemblies: Joint area assembly member neurons\n')
    JointOnly = false;
...
%             plot((2:length(D_.AssBestAssem.Members.Score{s})),D_.AssBestAssem.Members.Score{s}(2:end),':b','LineWidth',1.5);
    ...
    %% (4) Save results
    fnOut = sprintf('%sSyntheticAssemblies%s%s_AssemblyComparison_NoNoiseCorrs.mat',pat,filesep,fname);
