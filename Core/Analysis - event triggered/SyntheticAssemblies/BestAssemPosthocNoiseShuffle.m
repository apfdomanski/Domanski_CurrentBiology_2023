function BestAssemPosthocNoiseShuffle(iFile)
%% %%%%%% PREAMBLE %%%%%%
if isstr(iFile)
    iFile = str2num(iFile);
end
fprintf('running on File no. %d\n',iFile);
...
TrialsToUse     = 'Correct'; %'Correct','Error'
...
%     end
    ...
    for iDelay =2:3%1:length(Delays_)
        switch TrialsToUse
            case 'Correct'
    ...
    %% Load previous results of unshuffled assemblies
    fn = fullfile(pat,'SyntheticAssemblies',[fname,'_AssemblyComparison_redux7.mat']);
        temp = load(fn,'D_');
        D_ = temp.D_; 
        clear temp
    %% Decoding from BEST ASSEMBLIES with noise correlations smashed
        
        % All neurons
        fprintf('Calculating best assemblies: All units without noise correlations\n')
        unitSelection = D_.AssBestAssem.All.bestComboRanks;
        D_.AssBestAssem.All_noNoiseCorrs = MakebestEnsembleAssemsPosthoc(B.usel_out,Ltrials_,Rtrials_,20,false,true,noTrials,nRepsNoise,unitSelection);
        
        % Cell assembly member neurons
        fprintf('Calculating best assemblies: Assembly member neurons without noise correlations\n')
        unitSelection = D_.AssBestAssem.Members.bestComboRanks;
        D_.AssBestAssem.Members_noNoiseCorrs = MakebestEnsembleAssemsPosthoc(globalmemberUnitsReal,Ltrials_,Rtrials_,20,false,true,noTrials,nRepsNoise,unitSelection);
        ...
        unitSelection = D_.AssBestAssem.Nonmembers.bestComboRanks;
        D_.AssBestAssem.Nonmembers_noNoiseCorrs = MakebestEnsembleAssemsPosthoc(globalnonmemberUnitsReal,Ltrials_,Rtrials_,20,false,true,noTrials,nRepsNoise,unitSelection);
        ...
        unitSelection = D_.AssBestAssem.JointMembers.bestComboRanks;
        %     JointOnly = false;
        ...
        fprintf('Calculating best assemblies: Joint area assembly member neurons without noise correlations\n')
        D_.AssBestAssem.JointMembers_noNoiseCorrs = MakebestEnsembleAssemsCrossShufflePosthoc(JointMemberUnitsReal,        ...
            Ltrials_,Rtrials_,                 ...
            20,                                ...
            [1,1,1],                           ...
            noTrials,                          ...
...
%         temp.D_.AssBestAssem = D_.AssBestAssem;
%         D_ = temp.D_; clear temp
...
%             plot((2:length(D_.AssBestAssem.Members.Score{s})),D_.AssBestAssem.Members.Score{s}(2:end),':b','LineWidth',1.5);
    ...
    %% (4) Save results
    fnOut = sprintf('%sSyntheticAssemblies%s%s_AssemblyComparison_redux7PosthocShuffle.mat',pat,filesep,fname);
