function DecodingDuringDelayCrossTemporalBatch3JA(Target,iFile)
if isstr(iFile)
    iFile = str2num(iFile);
end
%% %%%%%% PREAMBLE %%%%%%
...
if iFile>length(fileList)
    disp(sprintf('Terminated: %d files available for target ''%s'', file no. %d requested.',length(fileList),Target,iFile))
    return
end
