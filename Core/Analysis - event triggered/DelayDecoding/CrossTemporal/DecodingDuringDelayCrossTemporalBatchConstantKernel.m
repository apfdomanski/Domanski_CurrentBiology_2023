function DecodingDuringDelayCrossTemporalBatchConstantKernel(iFile)
if isstr(iFile)
    iFile = str2num(iFile);
end
%% %%%%%% PREAMBLE %%%%%%
...
for iArea = 1%:length(Areas)
    ...
    if iArea < 3
        ...
        load(sprintf('%sKDE_binsTaskonly_2500mskernel%s%s_%s_iFR50_behavOnly.mat',pat,filesep, fname,Areas{iArea}));
        iFR_ = iFR;
    else
        U_{1} = load(sprintf('%sKDE_binsTaskonly_2500mskernel%s%s_%s_iFR50_behavOnly.mat',pat,filesep, fname,Areas{1}));
        U_{2} = load(sprintf('%sKDE_binsTaskonly_2500mskernel%s%s_%s_iFR50_behavOnly.mat',pat,filesep, fname,Areas{2}));
    ...
    %% Save results
    if exist('D_')
        fnOut = sprintf('%sMixedSelectivity%s%s_%s_DelayDecoding_UnitSpan_ErrorBounds_2500msKernel.mat',pat,filesep,fname,Areas{iArea});
