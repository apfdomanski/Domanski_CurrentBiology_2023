function DecodingDuringDelayVsBehavJobArrayGroups(iFile)
if isstr(iFile)
    iFile = str2num(iFile);
end
%% %%%%%% PREAMBLE %%%%%%
...
UnitSelection = 'groups'; %{'pairs','groups','all'}
groupSize = 5;
Areas = {'PFC','HP','Joint'};
color_={'b','r','g'};
...
%                                 CVE{i,1} = DecodeCVE(FR(:,rs{i}),evt0,0.05);
                                ...
                                [CVE_,~,~,~,~,CVE_shuffledHL] = RunDecodeGroups(1:groupSize_,10,L_,R_,inf,'max');
                                CVE{i,1}=CVE_';
