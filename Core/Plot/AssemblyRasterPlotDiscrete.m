function AssemblyRasterPlotDiscrete(Factor,Rates,Units,Events,plot_win)
...
for iArea = 1:3
    ...
    for iAss=1:size(Factor.FSC{iArea},2)
        ...
        plot(Factor.TmtxS(idx),Factor.FSC{iArea}(idx,iAss)-iAss/2-10*iArea,'color',colorscale{iArea}(iAss,:),'LineWidth',1)
        %%%%
    end
...
plot([plot_win(1)+0 plot_win(1)+20],[-6 -6],'LineWidth',2,'Color','k')
text(plot_win(1)+10,-4,'10s','HorizontalAlignment','center')
