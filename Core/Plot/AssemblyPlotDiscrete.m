function AssemblyPlotDiscrete(Factor,Rates,Units,Events,plot_win)
...
offset = 2;
for iArea = AreasToPlot

    for iAss=1:size(Factor.FSC{iArea},2)
        %%%%
        % plot the factor activations:
        idx = Factor.TmtxS>=plot_win(1) & Factor.TmtxS<=plot_win(2);
        plot(Factor.TmtxS(idx),Factor.FSC{iArea}(idx,iAss)-iAss*offset-5*iArea,'color',colorscale{iArea}(iAss,:),'LineWidth',1)
        %%%%
    end
