function [TmtxS,iFRs,EvtLs,EvtTs,usel_out]=SelTrials(fn,twin,SelectedUnits,iFRtype,trials)
...
if nargin<5 || isempty(trials),   trials='corr'; end; 
...
for f=1:length(fn0) % across brain regions: PFC, HP
    ...
    if ~isempty(strfind(trials,'all')) % strncmp(trials,'all',3)
