function [TmtxS,iFRs,usel_out]=SelTrialsCellsWholeTrial(fn,twin,minFR,critCvBW)
...
for f=1:length(fn0) % across brain regions: PFC, HP
    ...
    EvtTs = sortrows(twin,1);
    iFRs{f}=cell(1,size(EvtTs,1)); TmtxS{f}=iFRs{f};
    for i=1:size(EvtTs,1)
        k=find(Tmtx>=EvtTs(i,1) & Tmtx<=EvtTs(i,2));
        iFRs{f}{i}=iFR0(k,:)';
        TmtxS{f}{i}=Tmtx(k)';
    end
end
