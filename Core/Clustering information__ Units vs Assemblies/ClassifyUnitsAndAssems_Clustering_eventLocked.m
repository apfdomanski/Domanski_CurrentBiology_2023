...
if ispc
    pat{1}  = 'C:\Analysis\AssemblyAnalysis\raw\MixedSelectivity_LONG\LeftvsRight\'; % Location of decoding information
    pat{2} = 'C:\Analysis\AssemblyAnalysis\raw';                                    % location of raw spike times
    pat{3} = [pat{2} filesep 'KDE_binsTaskonly'];                                   % location of calculated firing rates
else

end
cd(pat{1})
RatList = dir([pat{2} filesep '*LONG*.mat']);
...
clear clusters result param
clusters.data.X=Group.AssemFeatures{3}';
