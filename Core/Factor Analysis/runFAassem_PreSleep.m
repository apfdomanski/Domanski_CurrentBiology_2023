...
%   [TmtxS,FiringRates,EvtLs,EvtTs,unit_IDs] = SelCellsSleep(fn,twin,minFR,critCvBW,iFRtype)
 ...
 fnOut=[pat separator target separator Name{f} '_test'];
...
for f=1:length(A)
    ...
    for event_id=1:length(TmtxS{1}), TmtxS{1}{event_id}=TmtxS{1}{event_id}([1:ko end-ko+1:end]); end;
...
for f=1:length(A)
    ...
    fnOut=[pat separator target separator Name{f} '_FSC']; load(fnOut);    fn=[pat A(f).name];
...
for f=1:length(A)
    ...
    for s=1:length(FSC)
        ...
        for Assem_id=1:nAssems
            ...
            plot_factor = 100; % downsampling factor
            n = find(abs(FL{s}{nassem{s}(3)}(:,Assem_id))>ciHld(s,3));
...
%             title(['#units: ' num2str(n')]);
    ...
    for s=1:length(FSC)
        [nTimePoints,nAssems]=size(FSC{s});
        plot_factor = 10; % downsampling factor
        for Assem_id=1:nAssems
            t = (1:ceil(nTimePoints/plot_factor))*plot_factor*0.05+0.05/2;
