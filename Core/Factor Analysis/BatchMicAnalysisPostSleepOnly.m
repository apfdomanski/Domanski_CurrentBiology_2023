...
for iFile =1:length(fileList)
    ...
    epoch = [max(times_)+30, max(cell2mat(M_))];
...
%     plot(x,3+0.*x,'.k')
    ...
    for s =1:3
        try
            mkdir([pat 'PostSleeptemp_' strtok(fname,'.') '_' Areas{s}])
            
            cd([pat 'PostSleeptemp_' strtok(fname,'.') '_' Areas{s}])
            ...
            fname_ = [pat 'MICresultsPostSleep' filesep strtok(fname,'.') '_' Areas{s} '_MICresultsPostSleep.mat'];
            ...
            % Save
            fname_ = [pat 'MICresultsPostSleep' filesep strtok(fname,'.') '_' Areas{s} '_AssembliesPostSleep.mat'];
