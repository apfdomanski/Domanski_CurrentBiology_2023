...
for iFile =1:length(fileList)
    ...
    epoch = [min(cell2mat(M__)),min(times_)-30];
...
%     plot(x,3+0.*x,'.k')
    ...
    for s =1:3
        try
            mkdir([pat 'PreSleeptemp_' strtok(fname,'.') '_' Areas{s}])
            
            cd([pat 'PreSleeptemp_' strtok(fname,'.') '_' Areas{s}])
            ...
            fname_ = [pat 'MICresultsPreSleep' filesep strtok(fname,'.') '_' Areas{s} '_MICresultsPreSleep.mat'];
            ...
            % Save
            fname_ = [pat 'MICresultsPreSleep' filesep strtok(fname,'.') '_' Areas{s} '_AssembliesPreSleep.mat'];
