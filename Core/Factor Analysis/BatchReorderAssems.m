...
elseif ismac
    addpath(genpath('/Users/aleksanderdomanski/Dropbox/MATLAB/PC MATLAB Path/AssemblyCode/Eleonora/Mic'))
    pat = '/Volumes/HDD2/DNMTP/raw/';
%     pat = '/Volumes/HDD2/DNMTP/raw/MICresults/MICresultsTask/';
else
...
display='raw';%'raw'; 'clustered'
display='ordunit'; % 'clustered' 'ordunit'
...
%             [assemblies]=Main_assemblies_detection(spM{s}, MaxLags, BinSizes,alph,Dc,No_th,O_th,bytelimit);
            ...
            fname_ = [pat 'MICresults/MICresultsTask' filesep strtok(fname,'.') '_' Areas{s} '_MICresultsTask.mat'];
            load(fname_)
            ...
            % Save
            fname_ = [pat 'MICresults/MICresultsTask' filesep strtok(fname,'.') '_' Areas{s} '_AssembliesTask_Unordered.mat'];
