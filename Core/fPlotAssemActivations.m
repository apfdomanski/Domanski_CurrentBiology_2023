function fPlotAssemActivations(Ass,plot_flag)
target = 'Homecage';
%% Optional plotting
if plot_flag
...
for s = 1:length(Ass.titles)      % across all areas/assem types
...
for s = 1:length(Ass.titles)      % across all areas/assem types
    ...
    subplot(1,length(Ass.titles),s), hold on
    title( Ass.titles{s})
